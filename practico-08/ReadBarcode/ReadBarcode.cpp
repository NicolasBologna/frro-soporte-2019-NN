...
void OutputResult(CBarcodeReader& reader,int errorcode,float time)
{
	...
	{
                snprintf(pszTemp, 4096, "False", time);
		printf(pszTemp);
		free(pszTemp);
	...
	for (int iIndex = 0; iIndex < paryResult->resultsCount; iIndex++)
	{
                ...
                //printf(pszTemp);
                snprintf(pszTemp, 4096, "%s", paryResult->results[iIndex]->barcodeText);
		printf(pszTemp);
...
int main(int argc, const char* argv[])
{
	...
	reader.InitLicense("t0068NQAAABYTS2enqZpXZ3x1WGOEoYTJ037QHcsBl7TawKal/PhPgyedTGXIuDeTzUtmw3K1LUTxN6v2i8yZlRkM19B/ZVg=");
