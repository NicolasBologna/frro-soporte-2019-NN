...
class NegocioSocio(object):
    ...
    def regla_1(self, socio):
        ...
        :raise: DniRepetido
        :return: bool
        True -> el dni es único
        """
        if self.datos.buscar_dni(socio.dni) != None:
            raise DniRepetido('Ya hay un socio con ese DNI.')
        else:
            return True
    ...
    def regla_2(self, socio):
        ...
        if len(socio.nombre)>self.MIN_CARACTERES and len(socio.nombre)<self.MAX_CARACTERES and len(socio.apellido)>self.MIN_CARACTERES and len(socio.apellido)<self.MAX_CARACTERES:
            ...
            raise LongitudInvalida('El nombre y el apellido del socio deben tener mas de 3 y menos de 15.')
    ...
    def regla_3(self):
        ...
        if len(self.todos()) <= self.MAX_SOCIOS:
            ...
            raise MaximoAlcanzado('Se llegó al máximo de socios')
